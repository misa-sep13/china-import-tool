        ...
        <select value={targetId} onChange={e => setTargetId(e.target.value)} style={selectStyle}>
          ...
          {targets.map(t => (
            <option key={t.id} value={t.id}>
              [{t.type === 'genre' ? 'ジャンル' : 'キーワード'}] {t.label}{!t.is_active ? '（停止中）' : ''}
            </option>
          ))}
        </select>
        <input
          ...
          <tbody>
            {targets.map(t => (
              <tr key={t.id} style={{ opacity: t.is_active ? 1 : 0.5 }}>
                <td style={tdStyle}>{t.type === 'genre' ? 'ジャンル' : 'キーワード'}</td>
    ...
    <div style={productCard}>
      ...
      <div style={{ padding: 10, display: 'flex', flexDirection: 'column', gap: 4, flex: 1 }}>
        ...
        <div style={{ fontSize: 12, color: '#6b7280' }}>
          ...
          {item.rank ? <span style={{ marginLeft: 8 }}>{item.rank}位</span> : null}
        </div>
