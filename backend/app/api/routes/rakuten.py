...
def _parse_permit_columns(text: str) -> list[dict]:
    """輸入許可書から申告欄ごとの関税率等を抽出する（共通サービスへ委譲）。"""
    return invoice_calc.parse_permit_columns(text)


def _match_items_to_columns(items: list[dict], columns: list[dict]) -> list[int | None]:
    """インボイス商品を許可書の申告欄にマッチングする（共通サービスへ委譲）。"""
    return invoice_calc.match_items_to_columns(items, columns)
...
def _calc_tariff_tax(
    ...
    """税率別計算（共通サービスへ委譲）。Amazon側と同じロジックを使う。"""
    return invoice_calc.calc_tariff_tax(
        items_with_totals,
        exchange_rate=data.exchange_rate,
        domestic_freight=data.domestic_freight,
        international_freight=data.international_freight,
        permit_cols_by_index={idx: data.items[idx].permit_col for idx, _ in items_with_totals},
        columns=columns,
        # 実際に払った納税額へ合わせて配る（欄の合計には端数が残っているため）
        actual_total_tax_jpy=data.import_tax_jpy or 0,
    )
