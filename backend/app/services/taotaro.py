...
def invoice_workbook(sid: int) -> bytes:
    ...
    if not name.endswith(".xlsx"):
        raise TaotaroError(
            ...
            "お手元のインボイスを使ってください")
    return raw
def list_orders(page: int = 1, limit: int = 100,
                start_time: Optional[int] = None) -> dict:
    """注文の一覧。発注済との突き合わせに使う。"""
    d = _request("/api/v1/orders", {"page": page, "limit": limit,
                                    "start_time": start_time})
    return {"items": [_order_brief(o) for o in (d.get("items") or [])],
            "has_more_pages": bool(d.get("has_more_pages"))}


def order_match_row(o: dict) -> dict:
    """注文1件を、荷受けの照合に渡せる形にする。

    管理番号を入れずに出した古い注文は out_id が空なので、
    URLと色・サイズで引くしかない。荷受けと同じ照合を通せば、
    同じURLの色違いも見分けられる。
    """
    props = o.get("sku_props") or []
    return {
...
def send_order_states(pages: int = 3) -> dict:
    """便ごとの状態。注文が届いたかどうかの判定に使う。"""
    out = {}
    for page in range(1, max(1, pages) + 1):
        d = list_send_orders(page=page, limit=100)
        for x in d.get("items") or []:
            if x.get("sid") is not None:
                out[x["sid"]] = x.get("state")
